...
const VessalOwerDetails = () => {
    ...
    const [shipCountError, setShipCountError] = useState("")
  ...
  */
    ...
    const onSubmit = () => {
        ...
        if (shipCount == "") {
            ...
            setShipCountError("Please enter your ship count..");
            errorCount++
    ...
    const Registersteptwo = () => {
        ...
        axios.post(baseUrl + 'registersteptwo', {
            ...
            if (response.data) {
                ...
                if (response.data) {
                    // console.log(response.data)
                    await AsyncStorage.setItem("vessalOwerDetails", JSON.stringify(response.data));
                    const vessalOwerDetails = await AsyncStorage.getItem("vessalOwerDetails")
                    const vessalOwer = JSON.parse(vessalOwerDetails);
                    if (vessalOwer !== null) {
        ...
        loader ? (<ActivityIndicator size={30} color={FONT_VOILET} style={{ flex: 1, justifyContent: "center", backgroundColor: BG_COLOR }} />) : (
            <ScrollView keyboardDismissMode="on-drag"
                ...
                <View style={{ marginHorizontal: 20 }}>
                    ...
                    <TextInput
                        style={Style.textinput}
                        placeholder='Enter your ship count'
                        keyboardType='default'
                        value={shipCount}
                        ref={shipCountRef}
                        onChangeText={(value) => {
...
export default VessalOwerDetails
